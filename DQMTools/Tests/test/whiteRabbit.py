...
class TestOptionParser:
    ...
    def defineOptions(self):
        ...
        self.group_general.add_option("-a",
                                      "--all",
                                      dest="a",
...
def notifyMe(mailFile):
    ...
    me_bis = 'suchandra.dutta@cern.ch'
    f = open(mailFile)
...
#    sys.exit(1)
    ...
    if parser.a():
        for test in range(1,numTests+1):
            ...
            a_thread = TestRunner(mainDir,ts,test, cwd, forceTest, Num=numev, Source=source, Client=client)
            a_thread.inspect()
...
#    print "Assembling final report on %s" % cwd
    if os.path.exists('%s' % cwd ):
        putAtTheEnd = ''
        command = 'find %s/*log -cnewer %d | sort -n -k 1 -t _' % (cwd, ts)
